...
class TimeExtractor():
    ...
    def get_all_files(self, directory_path):
    ...
    def parse_time_log(self, filename_path):
    ...
    def print_time_log(self, time_log, filename):
        ...
        if week_index != 0:
            ...
            total_week_minutes = total_week.total_seconds() % 3600 // 60
            print(f"From {start_date} to {date} : {int(total_week_hours)}h {int(total_week_minutes)}m")
        total_time = f"Total time: {int(total.total_seconds() // 3600)}h {int(total.total_seconds() % 3600 // 60)}m"
        print(total_time)
        return total_time
    ...
    def extract(self):
        ...
        with open(filename_path, encoding="utf8") as f:
            allfiles = self.get_all_files(f.readline())
        time_log = {}
        for filename in allfiles:
            filename_raw = os.path.basename(filename)
            #filename = 'data2.txt'  # Replace with your actual filename
            time_log[filename_raw] = self.parse_time_log(filename)
        total_time = self.print_time_log(time_log, filename)
        return total_time
